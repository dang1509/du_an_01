...
class SanPham{
    ...
    public function getAlbumAnhSanPham($id_san_pham)
    {
        try {
    ...
    public function search($danh_muc_id = null){
        try {
            $sql = 'SELECT san_phams.*, danh_mucs.ten_danh_muc FROM san_phams INNER JOIN danh_mucs ON san_phams.danh_muc_id= danh_mucs.id WHERE danh_muc_id=:id ORDER BY luot_xem DESC';
            $stmt = $this->conn->prepare($sql);
